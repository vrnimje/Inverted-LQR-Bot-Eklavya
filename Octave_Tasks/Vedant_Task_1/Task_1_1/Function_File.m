...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  jacobian_matrices = {};
  ################## ADD YOUR CODE HERE ######################
  jacobian_matrix = jacobian([x1_dot,x2_dot],[x1,x2]);
  for j = 1:length(x_1)
    jacobian_matrices{j} = double(subs(jacobian_matrix, [x1, x2], [x_1(j),x_2(j)]));
  endfor
  ############################################################
endfunction
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    n = 1;
    ################## ADD YOUR CODE HERE ######################
    eigen_values{k} = double(eig(matrix));
    for j = 1:numel(eigen_values{k})
